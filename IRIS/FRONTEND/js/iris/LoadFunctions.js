...
function LoadTipoDocumentoSelect(select, tienenulo) {
    $('#' + select).empty();
...
function LoadTipoEntidadSelect(select, tienenulo) {
    $('#' + select).empty();
...
function LoadPropuestaModalidad(select, tienenulo) {
  $('#' + select).empty();
...
function LoadPropuestaOrigen(select, tienenulo) {
  $('#' + select).empty();
...
function LoadPropuestaTipoUsuario(select, tienenulo) {
  $('#' + select).empty();
...
function LoadPropuestaEstado(select, tienenulo) {
  $('#' + select).empty();
...
function LoadAreaAcademicaSelect(select, tienenulo) {
  $('#' + select).empty();
...
function LoadPrestadorVinculoSelect(select, tienenulo) {
  return new Promise( (resolve, reject) => {
    ...
    $('#' + select).append('<option value="6">CONTRATISTA</option>');
...
// function LoadResolucionUGI(select, tienenulo) {
//   return new Promise( (resolve, reject) => {
//     $('#' + select).empty();
...
//     $('#' + select).append('<option value="1">2568</option>');
//     $('#' + select).append('<option value="2">1788</option>');
//     $('#' + select).append('<option value="3">1159</option>');
//     $('#' + select).append('<option value="4">2269</option>');
//     $('#' + select).append('<option value="5">1238</option>');
//     $('#' + select).append('<option value="6">1931</option>');
//     $('#' + select).append('<option value="7">2141</option>');
    
//     return resolve(true);
//   });  
// }

// Cambios ADDAVARGAS 27/05/2024
function LoadResolucionUGI(select, tienenulo) {
  $('#' + select).empty();
  let urlUGIResolucion = urlController + "UGI_Semestre/GetAllUGI_Semestre";

  return new Promise((resolve, reject) => {
    fetch(urlUGIResolucion, {
        method: 'GET',
        headers: { 'Authorization': 'Bearer ' + TokenIRIS }
    })
    .then(response => response.json())
    .then(data => {
        if (data.Ok) {
            let resoluciones = data.Data;
            console.log('Resoluciones:', resoluciones);
            if (tienenulo) {
              $('#' + select).append('<option value="">Seleccione</option>');
            }
            
            for (var i = 0; i < resoluciones.length; i++) {
              $('#' + select).append('<option value="' + resoluciones[i].id_ugisemestre + '">' + resoluciones[i].numresolucion + '</option>');
            }            
            return resolve(true);
        } else {
          return resolve(false);
...
//       // URL del endpoint que devuelve las resoluciones
//       let urlResolucion = urlController + "UGI_Semestre/GetDataTableUGI_Semestre";

//       fetch(urlResolucion, {
//           method: 'GET',
//           headers: { 'Authorization': 'Bearer ' + TokenIRIS } // Asegúrate de incluir cualquier header de autenticación necesario
//       })
//       .then(response => response.json())
//       .then(data => {
//           if (data && Array.isArray(data.data)) {
//               // Asumimos que 'data.data' contiene los registros
//               data.data.forEach(function(item) {
//                   if (item.numresolucion) { // Verifica que el item tenga el campo numresolucion
//                       $('#' + select).append(`<option value="${item.id_ugisemestre}">${item.numresolucion}</option>`);
//                   }
//               });
//               resolve(true);
//           } else {
//               console.error('No se recibieron datos válidos:', data);
//               resolve(false); // No hay datos válidos recibidos
//           }
//       })
//       .catch(err => {
//           console.error('Error al cargar las resoluciones:', err);
//           reject(err);
//       });
//   });
// }
...
//               console.log("Respuesta del servidor:", response);  // Añade esto para depurar

//               // Asegúrate de que response.data es un array antes de intentar iterar sobre él
//               if (response && Array.isArray(response.data)) {
//                   response.data.forEach(function(item) {
//                       if (item.numresolucion) {
//                           $('#' + select).append(`<option value="${item.id_ugisemestre}">${item.numresolucion}</option>`);
//                       }
//                   });
//                   resolve(true);
...
//                   reject("Formato de datos inesperado");
//               }
//           },
//           error: function(xhr, status, error) {
//               console.error("Error al cargar las resoluciones: ", status, error);
//               reject(error);
//           }
//       });
//   });
...
function LoadPublicacionTipoCorreccionselect(select, tienenulo) {
  return new Promise( (resolve, reject) => {
    ...
    $('#' + select).append('<option value="TRADUCCION">Traducción</option>');
      ...
      .then(data => {
        if (data.Ok) {
            ...
            if (tienenulo) {
             ...
             // $('#txtnumconsecutivo_DecVie_Consecutivo').val('');
...
function LoadManuscritosEstadoSelect(select, tienenulo) {
  $('#' + select).empty();
  let urlPublicaciones_DashBoard = urlController + "Publicaciones_DashBoard/EstadosPublicacionesManuscritosGetAll";    

  return new Promise( (resolve, reject) => {
    fetch(urlPublicaciones_DashBoard, {
        method: 'GET',
        headers: { 'Authorization': 'Bearer ' + TokenIRIS }
    })
    .then(response => response.json())
      .then(data => {
        if (data.Ok) {
            ...
            for (var i = 0; i < lstarea.length; i++) {
              $('#' + select).append('<option value="' + lstarea[i].idestadomanuscrito + '">' + lstarea[i].estadomanuscrito + '</option>');
            }            
            return resolve(true);;
        }
        else {
